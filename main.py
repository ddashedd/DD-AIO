...
def home():
    while 1:
        change_title(f"DD AIO | Home | {discord_name}")
        clear()
        ascii()
...
def modules():
    ...
    while 1:
        change_title(f"DD AIO | Modules | {discord_name}")
        clear()
        ascii()
...
def settings():
    while 1:
        load_config()
        change_title(f"DD AIO | Settings | {discord_name}")
        clear()
        ascii()
        if Checker.api_key:
...
def tools():
    while 1:
        load_config()
        change_title(f"DD AIO | Tools | {discord_name}")
        clear()
        ascii()
...
if __name__ == "__main__":
    ...
    [{cyan}DD AIO Download Page{reset}] https://DD.mickey-ye.lol
    [{cyan}DD AIO Discord Server{reset}] {discord_server}
